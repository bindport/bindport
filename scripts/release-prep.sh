...
release check gate, commits the version bump, pushes the branch, and opens a PR.
...
- MISE_TRUSTED_CONFIG_PATHS=\$PWD scripts/release-check.sh --version $new_version --allow-dirty"
