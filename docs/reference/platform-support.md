...
The supported Rust build floor (MSRV policy) is **Rust 1.98.0**. That is the
...
Release and compatibility changes must continue to compile and test on 1.98.0
until an announced MSRV increase.

The workspace uses Rust edition 2024 and declares `rust-version = "1.98"` for
...
| Cargo install | Source build on a supported Linux or macOS host using Rust 1.98.0 or newer |
...
all four prebuilt targets listed above with Rust 1.98.0.
