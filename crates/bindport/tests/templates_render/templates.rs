...
#[test]
fn templates_export_builtin_nginx_template() {
    let registry_path = temp_registry_path("templates-builtin-nginx-registry");
    let output = bindport_with_registry(&registry_path)
        .args(["templates", "export", "bindport-nginx"])
        .output()
        .expect("export built-in template");
    ...
    assert!(stdout.contains("Generated by BindPort"));
    assert!(stdout.contains("proxy_pass {{ route.target_url | tojson }}"));
}

#[test]
fn templates_export_builtin_haproxy_template() {
    let registry_path = temp_registry_path("templates-builtin-haproxy-registry");
    let output = bindport_with_registry(&registry_path)
        .args(["templates", "export", "bindport-haproxy"])
        .output()
        .expect("export built-in template");
    ...
    assert!(stdout.contains("Generated by BindPort"));
    assert!(stdout.contains("frontend {{ frontend_name }}"));
    assert!(stdout.contains("server {{ route.unique_slug }} {{ route.target_address | tojson }}"));
}
